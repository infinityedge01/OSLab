...
mmio_map_region(physaddr_t pa, size_t size)
{
	...
	size_t pgmin = ROUNDDOWN(base, PGSIZE);
	size_t pgmax = ROUNDUP(base + pa - ROUNDDOWN(pa, PGSIZE) + size, PGSIZE);
	size_t map_size = pgmax - pgmin;
	if(pgmax >= MMIOLIM){
		panic("mmio_map_region map memory exceed the MMIOLIM");
	}
	boot_map_region(kern_pgdir, base, map_size, ROUNDDOWN(pa, PGSIZE), PTE_PCD | PTE_PWT | PTE_W);
	base += map_size;
	return (void *) (base - map_size + pa - ROUNDDOWN(pa, PGSIZE));
}
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	void *ed = ROUNDUP((void*)(va + len), PGSIZE);
	void *i;
	pde_t *pde;
	pte_t *pte;
	for(i = st; i < ed; i += PGSIZE){
		if((uint32_t)i >= ULIM){
			user_mem_check_addr = (i == st) ? (uintptr_t)va : (uintptr_t)i;
			return -E_FAULT;
		}
		pde = &(env->env_pgdir[PDX(i)]);
		pte = pgdir_walk(env->env_pgdir, i, false);
		if(pte == NULL || ((uint32_t)(*pde) & perm) != perm || ((uint32_t)(*pte) & perm) != perm){
			user_mem_check_addr = (i == st) ? (uintptr_t)va : (uintptr_t)i;
			return -E_FAULT;
		}
	}
